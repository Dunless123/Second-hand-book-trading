$(function () {
	...
	$('.table-striped tbody').on('click', '.updateBtn1', function (val) {
		//获取行
		var row = $('.userPoint20 tr').index($(this).closest('tr'))
		//获取某列（从0列开始计数）的值
		var id = $('.userPoint20').find('tr').eq(row).find('td').eq(0).text()
		ssid = id
		$.ajax({
	...
	$('.table-striped tbody').on('click', 'tr', function (val) {
		location.href = '../buyDetail/buyDetail.html'
	})
